function ShortestLine=shortestpath_robust(DistanceMap,StartPoint,SourcePoint,Stepsize,Method)
...
% Modified by Gavin Taylor to prevent 2D execution. (Feb 2020) 
% rk4 occasionally returns Nan, possibly caused by structure of gradient volume
% Added a attempt at correction by reducing step size if this occurs 
% and a catch if it persists
...
while(true)
    ...
    switch(lower(Method))
        case 'rk4'
            ...
            if any(isnan(EndPoint))
                ...
                % If no movement, try simple.
                if all(StartPoint == EndPoint)
                    EndPoint=s1(StartPoint,DistanceMap);
                    % If still no movement, enlarge step size (default is 3)
                    if all(StartPoint == EndPoint)
                        EndPoint=s1_stepsize(StartPoint,DistanceMap,6);
                    end
                end
            end
        case 'euler'
    ...
    if any(isnan(EndPoint))
        error('Nan value in shortest path')
    end
