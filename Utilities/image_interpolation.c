 ...
 * Added: interpolate_3d_nearest_black & interpolate_3d_float_nearest_black
 * Modified: interpolate_3d_double_gray & interpolate_3d_float_gray
 *
 * To add: interpolate_3d_float_nearest & interpolate_3d_nearest
 */
...
// Gavin added function 
double interpolate_3d_nearest_black(double Tlocalx, double Tlocaly, double Tlocalz, int *Isize, double *Iin) {
    /*  Linear interpolation variables */
    ...
    if((xBas0>=0)&&(xBas0<Isize[0])&&(yBas0>=0)&&(yBas0<Isize[1])&&(zBas0>=0)&&(zBas0<Isize[2])) 
	{
...
// Gavin added function
float interpolate_3d_float_nearest_black(float Tlocalx, float Tlocaly, float Tlocalz, int *Isize, float *Iin) {
    /*  Linear interpolation variables */
    int xBas0, yBas0, zBas0;
	float Ipixel=0;
    ...
    if((xBas0>=0)&&(xBas0<Isize[0])&&(yBas0>=0)&&(yBas0<Isize[1])&&(zBas0>=0)&&(zBas0<Isize[2])) 
	{
		Ipixel=getcolor_mindex3_float(xBas0, yBas0, zBas0, Isize[0], Isize[1], Isize[2], Iin);
    }
    return Ipixel;
}
...
// Modified by Gavin to include nearest neighbour option
double interpolate_3d_double_gray(double Tlocalx, double Tlocaly, double Tlocalz, int *Isize, double *Iin, int cubic, int black){
    ...
    if(cubic==1) {
        ...
        if(black) { Ipixel=interpolate_3d_nearest_black(Tlocalx, Tlocaly, Tlocalz, Isize, Iin);}
        //nearest border pixel not implemented, just return zero
        else { Ipixel = 0;}
        //implement this later and include
        //else { Ipixel=interpolate_3d_nearest(Tlocalx, Tlocaly, Tlocalz, Isize, Iin);} 
    } else if (cubic==0) {
...
float interpolate_3d_float_gray(float Tlocalx, float Tlocaly, float Tlocalz, int *Isize, float  *Iin, int cubic, int black){
    ...
    if(cubic==1) {
        ...
        if(black) { Ipixel=interpolate_3d_float_nearest_black(Tlocalx, Tlocaly, Tlocalz, Isize, Iin);}
        //nearest border pixel not implemented, just return zero
        else { Ipixel = 0;}
        //implement this later and include
        //else { Ipixel=interpolate_3d_nearest(Tlocalx, Tlocaly, Tlocalz, Isize, Iin);} 
