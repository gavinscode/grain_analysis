function [srf]=nrbloft_nan(X,Y,Z,dir,mode)
...
% Modified by Gavin Taylor remove nans from input arrays (Feb 2020)
% Note, it should be possible to have different max Y values for each curve.
% To do: Parallelize merge notes, has large find.
...
if ~isstruct(X) && ~iscell(X)
    ...
    switch dir
        case 1
            for i=1:size(X,1)
                inds = find(~isnan(X(i,:)));
                u(i) = nrbinterp(X(i,inds),Y(i,inds), Z(i,inds),1);
            end
        case 2
            for i=1:size(X,2)
                inds = find(~isnan(X(:,i)));
                u(i) = nrbinterp(X(inds,i), Y(inds,i), Z(inds,i),1);
            end
        otherwise
...
% kToNIndex = cell(1,nn);
% for i = 1:nn
%     kToNIndex{i} = i*ones(1,length(k{i}));
% end
% kToNIndex = cell2vec(kToNIndex);
%[ku, ~, indexBackwards] = unique(cell2vec(k));
